...
export default function DynamicPage({ ids, components, ...rest } : Props) {
    return <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {ids.map(id => <DynamicComponent key={id} type={components[id].type} data={components[id]} selected={rest.inEditor && id === rest.selected} inEditor={rest.inEditor} onSelected={rest.inEditor ? rest.onSelected : undefined}/>)}
